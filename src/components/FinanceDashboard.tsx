  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        ...
        const sortedExpenses = jsonData.data.sort((a: Expense, b: Expense) => {
          const riskOrder: Record<string, number> = { 'flagged': 1, 'rejected': 2, 'approved': 3 };
          const aRisk = riskOrder[a.riskLevel?.toLowerCase() || 'flagged'] || 4;
          const bRisk = riskOrder[b.riskLevel?.toLowerCase() || 'flagged'] || 4;
          return aRisk - bRisk;
        });
